...
from training_server.executor import DockerExecutor, LABEL
...
class ExecutorTests(unittest.TestCase):
    def test_create_preserves_arguments_and_mounts_output(self):
        with tempfile.TemporaryDirectory() as root, patch('subprocess.run') as run:
            ...
            DockerExecutor().create('job-' + 'b' * 32, 'training:test', Path(root),
                                    ['python', 'train.py', 'space ; $(literal)'], '0')
    ...
    def test_stop_rejects_unmanaged_container(self):
        with patch.object(DockerExecutor, '_run', return_value=json.dumps([
                {'Config': {'Labels': {}}}])) as run:
            with self.assertRaises(ValueError):
                DockerExecutor().stop('a' * 64)
            self.assertEqual(run.call_count, 1)

    def test_start_checks_ownership_before_starting(self):
        with patch.object(DockerExecutor, '_run', side_effect=[json.dumps([
                {'Config': {'Labels': {LABEL: 'true'}}}]), '']) as run:
